...
# Put all the default code in a function that is run if the file is executed as a script, but not when imported.
def main():
    # make selections
    #############################################################
...
def calc_inf_inj_time_soler(input_csv='SOLER_SEP_catalog_PyOnset - WP2_multi_sc_event_list_draft.csv', output_csv=False, sw=400, clean_df=True, round_minutes=True):
    ...
    df['p25MeV onset solar wind speed (km/s)'] = pd.to_numeric(df['p25MeV onset solar wind speed (km/s)'], errors='coerce').astype(pd.Int64Dtype())
    df['e100keV onset solar wind speed (km/s)'] = pd.to_numeric(df['e100keV onset solar wind speed (km/s)'], errors='coerce').astype(pd.Int64Dtype())
    df['e1MeV onset solar wind speed (km/s)'] = pd.to_numeric(df['e1MeV onset solar wind speed (km/s)'], errors='coerce').astype(pd.Int64Dtype())
...
def update_soler_catalogue(input_csv='SOLER_SEP_catalog_PyOnset - WP2_multi_sc_event_list_draft.csv', output_csv="SOLER_SEP_catalogue.csv", datetime_source='Solar-MACH link', sw=400, clean_df=True, round_minutes=True, events='default'):
    """
    Runs all steps to update the SOLER SEP catalogue with new columns containing spacecraft coordinates and inferred injection times.

    Example:
    >> from make_multi_sc_plots_SERPENTINE_catalog import update_soler_catalogue
    >> df = update_soler_catalogue()
    """
    print("Note: This will run through the catalogue three times, so it might take a while! To speed up the process, remove the 'empty' events at the bottom of the catalogue before executing this function.")
    print("Running step 1/3: Obtaining spacecraft coordinates...")
    _ = get_sc_coords(input_csv=input_csv, output_csv=output_csv.split('.')[-2]+'_1.csv', datetime_source=datetime_source)
    print("Running step 2/3: Calculating inferred injection times...")
    _ = calc_inf_inj_time_soler(input_csv=output_csv.split('.')[-2]+'_1.csv', output_csv=output_csv.split('.')[-2]+'_2.csv', sw=sw, clean_df=clean_df, round_minutes=round_minutes)
    print("Running step 3/3: Converting PSP peak count-rates to fluxes...")
    df = convert_psp_soler(input_csv=output_csv.split('.')[-2]+'_2.csv', output_csv=output_csv, clean_df=clean_df, events='default')
    return df
