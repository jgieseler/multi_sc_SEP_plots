"""
This file has been modified to produce the figures for the catalogoue paper. Don't use it for another purpose!
"""
...
plot_p = True
save_fig = True
outpath = os.getcwd()  # 'plots'  # '/Users/dresing/Documents/Proposals/SERPENTINE_H2020/Cycle25_Multi-SC_SEP_Event_List/Multi_sc_plots'
...
for i in tqdm([30, 48]):  # standard TODO:
    ...
    """Manual loading of PSP electron data"""
    fpath = 'psp_data_priv_comm/'
    f_psp_mar_2022_1mev = fpath+"HET_electrons_240219_2022-03-28_60sec.csv"

    f_psp_mar_2022_100kev_w3 = fpath+"w3_chane_65-153keV_event222830.csv"
    f_psp_mar_2022_100kev_w7 = fpath+"w7_chane_65-153keV_event222830.csv"

    f_psp_feb_2023_100kev_w3 = fpath+"w3_chane_65-153keV_event230224.csv"
    f_psp_feb_2023_100kev_w7 = fpath+"w7_chane_65-153keV_event230224.csv"

    # 100 keV e
    df_psp_mar_2022_100kev_w3 = pd.read_csv(f_psp_mar_2022_100kev_w3, date_format='%Y-%jT%H:%M:%S.%f', header=27)
    df_psp_mar_2022_100kev_w7 = pd.read_csv(f_psp_mar_2022_100kev_w7, date_format='%Y-%jT%H:%M:%S.%f', header=27)
    df_psp_feb_2023_100kev_w3 = pd.read_csv(f_psp_feb_2023_100kev_w3, date_format='%Y-%jT%H:%M:%S.%f', header=28)
    df_psp_feb_2023_100kev_w7 = pd.read_csv(f_psp_feb_2023_100kev_w7, date_format='%Y-%jT%H:%M:%S.%f', header=28)

    # replace bad data
    df_psp_mar_2022_100kev_w3[df_psp_mar_2022_100kev_w3 == -1.00000e31] = np.nan
    df_psp_mar_2022_100kev_w7[df_psp_mar_2022_100kev_w7 == -1.00000e31] = np.nan
    df_psp_feb_2023_100kev_w3[df_psp_feb_2023_100kev_w3 == -1.00000e31] = np.nan
    df_psp_feb_2023_100kev_w7[df_psp_feb_2023_100kev_w7 == -1.00000e31] = np.nan

    # 1 MeV e
    df_psp_mar_2022_1mev = pd.read_csv(f_psp_mar_2022_1mev, header=8)
    df_psp_mar_2022_1mev["combined"] = df_psp_mar_2022_1mev["Year"]*1000 + df_psp_mar_2022_1mev["FP_DOY"]
    df_psp_mar_2022_1mev["date"] = pd.to_datetime(df_psp_mar_2022_1mev["combined"], format="%Y%j")
    df_psp_mar_2022_1mev["decimal_hour"] = (df_psp_mar_2022_1mev["FP_DOY"].values - df_psp_mar_2022_1mev["FP_DOY"].astype('int'))*24
    df_psp_mar_2022_1mev["date"] = df_psp_mar_2022_1mev.apply(lambda x: x["date"] + pd.DateOffset(hours=x["decimal_hour"]), axis=1)
    df_psp_mar_2022_1mev.index = df_psp_mar_2022_1mev.date
    df_psp_mar_2022_1mev[df_psp_mar_2022_1mev == -999] = np.nan

    # averaging energy channels
    psp_het_view = 'A'
    flux_psp_mar_2022_1mev = (df_psp_mar_2022_1mev[f'{psp_het_view} 0.71-1.19 MeV'] * 0.48 + df_psp_mar_2022_1mev[f'{psp_het_view} 1.19-1.41 MeV'] * 0.22) / (0.48+0.22)
    av_ch_psp_mar_2022_1mev = '0.71 - 1.41 MeV'

    # resampling
    df_psp_mar_2022_100kev_w3 = resample_df(df_psp_mar_2022_100kev_w3, averaging)
    df_psp_mar_2022_100kev_w7 = resample_df(df_psp_mar_2022_100kev_w7, averaging)
    df_psp_feb_2023_100kev_w3 = resample_df(df_psp_feb_2023_100kev_w3, averaging)
    df_psp_feb_2023_100kev_w7 = resample_df(df_psp_feb_2023_100kev_w7, averaging)
    flux_psp_mar_2022_1mev = resample_df(flux_psp_mar_2022_1mev, averaging)
    """Manual loading of PSP electron data"""
    ...
    if plot_e_100:
        ...
        if PSP:
            if i == 30:
                ax.plot(df_psp_mar_2022_100kev_w3.index, df_psp_mar_2022_100kev_w3.DATA.values*1000, color=psp_het_color, linewidth=linewidth,
                        label='PSP ISOIS-EPILO F (W3)\n'+psp_epilo_chstring_e,
                        drawstyle='steps-mid')
            if i == 48:
                ax.plot(df_psp_feb_2023_100kev_w3.index, df_psp_feb_2023_100kev_w3.DATA.values*1000, color=psp_het_color, linewidth=linewidth,
                        label='PSP ISOIS-EPILO F (W3)\n'+psp_epilo_chstring_e,
                        drawstyle='steps-mid')
            # if len(psp_epilo) > 0:
    ...
    if plot_e_1:
        ...
        if PSP:
            if i == 30:
                ax.plot(flux_psp_mar_2022_1mev.index, flux_psp_mar_2022_1mev.values, color=psp_het_color, linewidth=linewidth,
                        label='PSP ISOIS-EPIHI-HET A (sun)\n'+av_ch_psp_mar_2022_1mev,
                        drawstyle='steps-mid')
    ...
    if plot_p:
        ...
        if SOLO:
            if het and (len(ept_e) > 0):
                het_chstring_p = '25.09 - 41.18 MeV'
